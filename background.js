(() => {
    ...
    const handleMessage = (message, sender, respond) => {
    ...
    chrome.commands.onCommand.addListener(command => {
      if (command === 'sort_all_tabs') sortAllTabs();
      else if (command === 'sort_current_window') sortCurrentWindowTabs();
      else if (command === 'close_duplicates') closeDuplicateTabs();
    });
    ...
    const findDuplicateTabs = tabs => {
      ...
      const duplicateTabs = [];
      tabs.forEach(t => {
        if (tabUrls.has(t.url)) {
          duplicateTabs.push(t);
        } else {
          tabUrls.add(t.url);
        }
      });
